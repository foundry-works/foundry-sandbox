...
try:
    from addons.oauth_managers.opencode import OpenCodeKeyManager
except ImportError:
...
class CredentialInjector:
    ...
    def _get_container_id(self, flow: http.HTTPFlow) -> Optional[str]:
        ...
        Returns None if container identity is not available (backwards compat mode).
        """
        container_config = get_container_config(flow)
        if container_config:
            return container_config.container_id
        return None
    ...
    def request(self, flow: http.HTTPFlow) -> None:
        ...
        Uses container identity from flow.metadata for context-aware logging
        and per-container rules.
        """
        host = flow.request.host
        container_id = self._get_container_id(flow)
