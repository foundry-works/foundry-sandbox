...
merge_claude_settings() {
    ...
    # Merge settings in container using foundry_sandbox.claude_settings bridge
    docker exec -u "$CONTAINER_USER" "$container_id" \
        python3 -m foundry_sandbox.claude_settings merge "$container_settings" "$temp_host"
...
sync_opencode_foundry() {
    ...
    if file_exists "$template_path"; then
        ...
        if ! $run_fn docker exec -i -u "$CONTAINER_USER" \
            ...
            "$container_id" python3 -m foundry_sandbox.opencode_sync sync \
            "$template_dst" "$CONTAINER_HOME/.config/opencode/opencode.json"; then
            if [ "$quiet" != "1" ]; then
